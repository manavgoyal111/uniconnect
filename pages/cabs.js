...
export async function getServerSideProps(context) {
	...
	let products = await Product.find({ category: "cab" });
	let cabs = {};
	for (let item of products) {
		if (item.title in cabs) {
			if (!cabs[item.title].color.includes(item.color) && item.availableQty > 0) {
				cabs[item.title].color.push(item.color);
			}
			if (!cabs[item.title].size.includes(item.size) && item.availableQty > 0) {
				cabs[item.title].size.push(item.size);
			}
		} else {
			cabs[item.title] = JSON.parse(JSON.stringify(item));
			if (item.availableQty > 0) {
				cabs[item.title].color = [item.color];
				cabs[item.title].size = [item.size];
			} else {
	...
	return {
		props: { products: JSON.parse(JSON.stringify(cabs)) },
	};
}
